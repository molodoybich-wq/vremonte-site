(() => {
  ...
  // Desktop dropdown (Услуги / Отзывы / Контакты)
  const dd = document.getElementById("dd");
  const ddClose = document.getElementById("ddClose");
  const ddBtns = document.querySelectorAll(".nav__btn--dd");

  function closeDD() {
    if (!dd) return;
    dd.setAttribute("aria-hidden", "true");
    dd.classList.remove("open");
    dd.querySelectorAll("[data-dd-panel]").forEach(p => p.hidden = true);
    ddBtns.forEach(b => b.setAttribute("aria-expanded", "false"));
  }

  function openDD(key, btn) {
    if (!dd) return;
    dd.querySelectorAll("[data-dd-panel]").forEach(p => p.hidden = p.getAttribute("data-dd-panel") !== key);
    dd.setAttribute("aria-hidden", "false");
    dd.classList.add("open");
    ddBtns.forEach(b => b.setAttribute("aria-expanded", String(b === btn)));
  }
  ...
  if (dd && ddBtns.length) {
    ddBtns.forEach(btn => {
      btn.addEventListener("click", (e) => {
        ...
        const isOpen = dd.classList.contains("open") && btn.getAttribute("aria-expanded") === "true";
        if (isOpen) closeDD();
        else openDD(key, btn);
      });
    });
    ...
    dd.addEventListener("click", (e) => {
      if (e.target === dd) closeDD(); // click on backdrop
    });
    ...
    document.addEventListener("click", (e) => {
      ...
      const inBtn = e.target.closest(".nav__btn--dd");
      if (!inDD && !inBtn) closeDD();
    });
  }
  ...
  try {
    const accepted = localStorage.getItem("cookie-accepted") === "true";
    if (cookieNotice && !accepted) cookieNotice.hidden = false;
    ...
    if (cookieClose) cookieClose.addEventListener("click", () => {
      if (cookieNotice) cookieNotice.hidden = true;
    });
  } catch (e) {
    // if storage blocked, still allow close
    if (cookieNotice) cookieNotice.hidden = false;
    if (cookieClose) cookieClose.addEventListener("click", () => cookieNotice.hidden = true);
  }
